...
function render(b::CheckboxButton, cam::Camera, renderer)
    ...
    # Render checkbox
    render_checkbox_square(b.button, 6, SDL_Color(200,200,200, 255), cam, renderer)

    if b.toggled
        # Inside checkbox "fill"
        render_checkbox_square(b.button, 8, SDL_Color(100,100,100, 255), cam, renderer)
    end
end
...
function render_checkbox_square(b::Button, border, color, cam, renderer)
    ...
    screenPos = uiToScreen(topLeft, cam)
    rect = SDL_Rect(screenPos.x, screenPos.y, screenScaleDims(checkbox_radius*2, checkbox_radius*2, cam)...)
    SDL_SetRenderDrawColor(renderer, Int64(color.r), Int64(color.g), Int64(color.b), 255)
    SDL_RenderFillRect(renderer, Ref(rect) )
end
